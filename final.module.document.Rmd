...
<p class="text-primary">In this module, I will walk through replicating the summary statistics, Generalize Linear Model and Survival analyses in Finkbeiner et al. 2017 and recreating Figure 1 of the aforementioned paper.</p>
...
At first glance these datasets look the same, but the binomial dataset is substantially longer 
...
In her 2017 publication, Susan Finkbeiner reports, "Our primary predation experiment tested whether Adelpha species with locally convergent wing patterns experience reduced preda- tion rates compared to novel and known palatable butterfly prey. Two-thousand total artificial butterfly facsimiles were used in this study, which yielded 189 combined avian attacks: 52 on Junonia (10.4% attacked), 30 on A. iphiclus (6.0% attacked), 56 on A. serpa (11.2% attacked), and 51 on A. leucopthalma (10.2% at- tacked, Fig. 1A)." 

I will replicated these summary statistics below. 

We can use the subset function to subset only the data that represent attacks on butterfly facsimiles. This can be useful when working with large datasets like this that contains a lot of zeros. We can also use the aggregate function to look at the total attacks on each species 
...
We can also calculate the attack rates on each species by dividing the total number of attacks by the total number of facsimilies, which is 500 for each species
...
```{r}
no.zero <- subset(count.data, attacks >= "1", drop = FALSE)
a <- table(no.zero$attacks, no.zero$species)
rates <- (a/500)*100
rates
```

We can also use the subset and aggregate functions to list the attacks totals for each species by day 
...
overall glm with all comparisons as reported in the legend of Figure 1A. 
```{r}
...
She then reported what seemed to be pairwise glms, but I couldn't find a post hoc test or anything that would give me pairwise comparisons in a glm so I subsetted the specific species and ran additional glms for each. 

In her paper, Finkbeiner reports, "We found substantial evidence that attack rates vary between the facsimile phenotypes. Using a general linear model with Pois- son distribution, we found that A. iphiclus was attacked signifi- cantly less than the other three facsimile phenotypes (z-value = 2.399, d.f. = 399, P = 0.0164 vs J. evarete; z-value = 2.579, d.f. = 399, P = 0.0058 vs A. serpa; z-value = 2.306, d.f. = 399, P = 0.0211 vs A. leucopthalma). The remaining facsimile phe-
notypes, J. evarete, A. serpa, and A. leucopthalma, were attacked at equal rates (all P > 0.6)." 

evarete-iphiclus comparison 
```{r}
...
leucopthalma-iphiclus comparison 
```{r}
...
leucopthalma-evarete comparison 
```{r}
...
# Figure 1(A)
I struggled with creating the error bars and inserting the species images in this graph. I calculated the 95% confidence intervals associated with the glm above. But this analysis was performed on the attack totals, not the attck rates/proportions. I am not sure if she made a separate dataset to run a glm on attack rates, but (to the best of my understanding) if she did make a new dataset with proportions, it would no longer be a Poisson distribution. 

Also, in order to use ggimage, I had to make a new dataset, which I think make it more difficult to add in the error bars. 
...
The next preliminary step is creating a subsetting object of just iphiclus. we will use this in the ggplot code to insert an * to represent that our analyses indicate that attack rates on iphiclus are significantly lower. 
```{r}
...
b <- b + geom_image(aes(image=image, y = 0.02), size=.2) ##y= specifies the location of the image                                                             along the y axis 
...
b <- b + scale_y_continuous(breaks = seq(0.0, 0.12, 0.02)) ##seq() command specifies seq(to, from,                                                              by=)
b
```
...
```{r}
coxph(Surv(day, attacks) ~ species, data=binomial.data)
```

I also found this code that should give me pairwise comparisons within the survival package but my R isn't recognizing surv.multcomp and I don't know why... 
...
surv.multcomp(Surv(day, attacks) ~ species, data = data, strata = NULL, type = "coxph", p.method = "fdr")
```
